...
// rewrite the below using a callback
...
// void handle_motor_power_status(SerializedCanMessage &msg) {
//     MotorControllerPowerStatus status{};
//     status.deserialize(&msg);

//     uint16_t motor_rpm = status.motor_rpm;

//     // TODO: feed motor_rpm into cruise control
// }

void forward_motor_can_message(SerializedCanMessage &msg)
{
    if (msg.id == MotorControllerPowerStatus::get_message_ID())
    {
        MotorControllerPowerStatus status{};
        status.deserialize(&msg);
    ...
    {
        MotorControllerDriveStatus status{};
        status.deserialize(&msg);
        main_can.write(&status);
    }
