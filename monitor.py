...
#!/usr/bin/env python3
# """Monitor that displays incoming COBS frames as hex."""
...
# def main():
#     port = get_correct_port()
#     ser = Serial(port, baudrate=921600, timeout=None)
#     print(f"Connected to {port}. Listening for COBS frames...\n")
...
#             b = ser.read(1)
#             if b[0] == 0x00:
#                 # 0x00 delimiter = end of COBS frame
#                 hex_str = " ".join(f"{x:02X}" for x in buf) if buf else "<empty>"
#                 print(f"Frame {frame_num}: [{len(buf)} bytes] {hex_str}")
#                 frame_num += 1
#                 buf.clear()
#             else:
#                 buf.append(b[0])
#     except KeyboardInterrupt:
#         ser.close()
#         print(f"\nDone. {frame_num} frames received.")

# if __name__ == "__main__":
#     main()

#!/usr/bin/env python3
"""Monitor that receives COBS frames and echoes them back for loopback testing."""
...
def main():
    port = get_correct_port()
    ser = Serial(port, baudrate=921600, timeout=None)
    print(f"Connected to {port}. Echoing COBS frames...\n")
    ...
    try:
        while True:
            ...
            if b[0] == 0x00:
                ...
                # Echo back: encoded bytes + 0x00 delimiter
                ser.write(buf)
                ser.write(b'\x00')
                ser.flush()
                print(f"  -> echoed back")
        ...
        ser.close()
        print(f"\nDone. {frame_num} frames received and echoed.")
