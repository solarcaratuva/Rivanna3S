...
void MX_USART6_UART_Init(uint32_t baudrate) // <-- This line changed when importing
{
  ...
  huart6.Init.BaudRate = baudrate; // <-- This line changed when importing
...
void HAL_UART_MspInit_custom(const USART_TypeDef* uartHandle, Pin pin, uint8_t af) // <-- This line changed when importing
{
  ...
  */
  ...
  else if(uartHandle == USART6) // <-- This line changed when importing)
  {
